...
/* Reads must never come off a cached copy, and on this store that rules out
 * every fixed-pathname scheme:
 *
 *   - A public blob is served from its CDN URL, and `cacheControlMaxAge`
 *     cannot go below 60s. Overwrite the path, read it back, and you get the
 *     PRE-write JSON — the just-created campaign that 404s for a minute.
 *   - `useCache: false` does not help. The SDK honours it *only* for private
 *     blobs (it appends `?cache=0` under `access === "private"` and silently
 *     drops the flag otherwise), and this store is public: private writes are
 *     rejected with "Cannot use private access on a public store".
 *   - Cache-busting the URL does not help either. The blob CDN ignores unknown
 *     query parameters, so `?t=123` resolves to the same cached object.
 *
 * So nothing is ever overwritten. Every save writes a NEW immutable pathname
 * stamped with the write time, and a read lists the prefix through the Blob
 * HTTP API — `list()` is a control-plane call, not a CDN fetch, so it sees a
 * write immediately — then fetches the newest entry. That URL has never been
 * requested before, so there is no cached copy of it to serve, and its body
 * never changes afterwards, which makes it safe for the CDN to keep.
 */
const versionPrefix = (key: Key) => `state/${key}/`;
/** Where collections lived when each one was a single overwritten blob. */
const legacyPath = (key: Key) => `${key}.json`;

/** Timestamp zero-padded so lexicographic pathname order == write order. */
function versionedPath(key: Key): string {
  const stamp = String(Date.now()).padStart(16, "0");
  const token = Math.random().toString(36).slice(2, 10);
  return `${versionPrefix(key)}${stamp}-${token}.json`;
}
...
async function blobReadJson<T>(key: Key): Promise<BlobRead<T>> {
  ...
  const newest = byNewest(blobs)[0];
  if (newest) {
    const res = await fetch(newest.url, { cache: "no-store" });
    // A 404 here means the version was pruned between listing and fetching,
    // not that the collection is absent — surfacing it as "missing" would let
    // loadCache seed over live data.
    if (!res.ok) {
      throw new Error(`[store] fetching ${newest.pathname} failed: ${res.status}`);
    }
    return { found: true, data: (await res.json()) as T };
  }
  const legacy = await readLegacyJson<T>(key);
  return legacy ? { found: true, data: legacy } : { found: false };
}

/**
 * Reads the pre-versioning blob, or null when there isn't one. Only reached
 * before a collection's first write under the new scheme; that blob has been
 * sitting untouched for well over the cache TTL, so a cached read is fine.
 */
async function readLegacyJson<T>(key: Key): Promise<T | null> {
  ...
  try {
    ...
    throw err; // a real failure — never let it read as "missing"
  }
  const res = await fetch(url, { cache: "no-store" });
  if (res.status === 404) return null;
  if (!res.ok) throw new Error(`[store] legacy fetch for ${key} failed: ${res.status}`);
  return (await res.json()) as T;
}

async function blobWriteJson(key: Key, data: unknown): Promise<void> {
  const { put } = await import("@vercel/blob");
  await put(versionedPath(key), JSON.stringify(data, null, 2), {
    access: "public",
...
/**
 * Immutable writes accumulate, so drop the stale ones. Keeps several rather
 * than one so a request that listed a moment ago can still fetch what it saw.
 * Best-effort: a failed prune costs storage, never correctness.
 */
async function pruneVersions(key: Key, keep = 5): Promise<void> {
  try {
    const { list, del } = await import("@vercel/blob");
    const { blobs } = await list({ prefix: versionPrefix(key) });
    const stale = byNewest(blobs).slice(keep);
    if (stale.length) await del(stale.map((b) => b.url));
  } catch (err) {
    console.error(`[store] pruning old ${key} versions failed`, err);
  }
}
